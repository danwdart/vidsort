...
export default async (req, res) => {
    ...
    const playlists = await cursor(promisify(youtube.playlists.list),
        {
            maxResults: 50,
            mine: true,
            part: `snippet`
    ...
    for (let i = 0; i < playlists.data.items.length; i++) {
        ...
        if (config.pointlessTags.find(tag => playlist.snippet.title === `VidSort: ${tag}`)) {
            ...
            console.warn(`Would delete playlist ID ${playlist.id}`);
            await promisify(youtube.playlists.delete)(
                {
                    id: playlist.id
                }
            );
        ...
        const playlistItems = await cursor(promisify(youtube.playlistItems.list),
            {
                part: `snippet`,
                maxResults: 50,
        ...
        for (let j = 0; j < playlistItems.data.items.length; j++) {
            ...
            const videoResponse = await promisify(youtube.videos.list)(
                {
                    part: `snippet`,
    ...
    for (const tag in videosByTag) {
        ...
        if (foundPlaylist) {
            ...
            const createPlaylistResponse = await promisify(youtube.playlists.insert)(
                {
                    "part": "snippet",
                    "resource": {
                        "snippet": {
                            "title": playlistName,
                            "description": `Created by JolHarg VidSort for the tag "${tag}"`
                        }       
                    }
                }
            );
        ...
        for (let l = 0; l < videoList.length; l++) {
            ...
            const insertPlaylistItemResponse = await promisify(youtube.playlistItems.insert)(
                {
                    part: `snippet`,
                    resource: {
                        snippet: {
